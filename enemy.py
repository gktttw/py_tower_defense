...
class AbstractEnemy(Unit):
    ...
    def __init__(self, grid_size=(.2, .2), grid_speed=1 / 12, health=100):
        ...
        # attribute for ice tower
        self.if_slowed = False
        self.immune_slow = False
...
class AdvanceEnemy(AbstractEnemy):
    """Advance type of enemy immune to ice tower, become faster when health ls low and change color"""
    name = "Advance Enemy"
    colour = 'black'

    points = 30

    def __init__(self, grid_size=(1.5, 1.5), grid_speed=1/60, health=10000):
        super().__init__(grid_size, grid_speed, health)
        self.immune_slow = True
    ...
    def step(self, data):
        ...
        # speed and size depends on health
        
        if self.percentage_health() < 0.2:
            movement = self.grid_speed*9
            self.colur = "yellow"
            self.size = (12,12)
            
        elif self.percentage_health() < 0.5:
            movement = self.grid_speed*3
            self.colour = "green"
            self.size = (42, 42)
        else:
            movement = self.grid_speed
