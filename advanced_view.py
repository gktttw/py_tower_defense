...
from tower import AbstractTower, MissileTower, PulseTower, SimpleTower, \
    AbstractObstacle, Missile, Pulse, IceTower
from enemy import AbstractEnemy, AdvanceEnemy
...
class TowerView(SimpleView):
    ...
    @classmethod
    def _draw_ice(cls, canvas: tk.Canvas, tower_: IceTower):
        """Draws an ice tower"""
        ...
        outer_points = [x+x_diameter/3*cell_size, y, x, y+y_diameter/3*cell_size,
                 x-x_diameter/3*cell_size, y, x, y-y_diameter/3*cell_size]
        tags.append(canvas.create_polygon(outer_points, fill='#3366ff', tag='tower'))

        tags.append(canvas.create_rectangle(x-x_diameter/5*cell_size, y-y_diameter/5*cell_size,
                                        x+x_diameter/5*cell_size, y+y_diameter/5*cell_size
                                        , fill='#3366ff', tag='tower', outline='#3366ff'))
...
class EnemyView(SimpleView):
    ...
    def _draw_simple(cls, canvas: tk.Canvas, enemy: AbstractEnemy):
        ...
        # when it's slowed
        if enemy.if_slowed and not enemy.immune_slow:
            fill = canvas.create_arc(top_left, bottom_right, tags='enemy',
                                    fill="#66ffff", start=45, extent=-extent,
                                    outline='')
        else:
            fill = canvas.create_arc(top_left, bottom_right, tags='enemy',
